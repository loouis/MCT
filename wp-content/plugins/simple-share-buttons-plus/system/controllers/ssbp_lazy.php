...
class ssbpShareButtons
{
    ...
    public function get_ssbp_buttons($urlCurrentPage, $strPageTitle, $urlShortened = '', $post_id = null, $all = false)
    {
        // variables
    ...
    public function get_ssbp_share_counts()
    {
        ...
        if ($counts) {
            ...
            $hasCounts = array(
                'facebook', 'google', 'linkedin', 'pinterest', 'reddit', 'stumbleupon', 'tumblr', 'twitter', 'vk',
            );
    ...
    public function ssbp_facebook_count()
    {
        ...
        if ($this->ssbp_settings['share_api'] == 'Y') {
            ...
            $htmlSSBPAPI = wp_remote_post('https://api.simplesharebuttons.com/v1/facebook_share_count', array(
                'method' => 'POST',
                'timeout' => 4,
                'body' => array(
    ...
    public function ssbp_twitter()
    {
        ...
        if ($this->ssbp_settings['counters_enabled'] == 'Y') {
            ...
            if ((int)$this->ssbpShareCountData['twitter'] >= intval($this->ssbp_settings['min_shares'])) {
                $twitter_share_count = '<span class="ssbp-total-twitter-shares ssbp-each-share">' . ssbp_format_number($this->ssbpShareCountData['twitter']) . '</span>';
            }
        }
        ...
        // twitter share link
        $twitterLink = '<a href="https://twitter.com/share?url=' . ($this->urlShortened == '' ? $this->urlCurrentPage : $this->urlShortened) . '&amp;text=' . $twitterShareText . '&amp;tags=' . $this->ssbp_settings['twitter_tags'] . $ssbpTwitterVia . '" class="ssbp-btn ssbp-twitter ssbp-twitter--standard"' . ($this->ssbp_settings['rel_nofollow'] == 'Y' ? 'rel="nofollow"' : null) . ' data-ssbp-title="' . $this->strPageTitle . '" data-ssbp-url="' . $this->urlCurrentPage . '" data-ssbp-site="Twitter" ' . ($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\'' . $this->urlCurrentPage . '\', \'Twitter\'); return false;"' : null) . '>';
        ...
        // put button together
        $twitterButtons = $twitterLink.$twitterButton.'</a>'.$twitter_share_count;
