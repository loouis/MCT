...
function wp_review_enqueue() {
	...
	if(is_singular()) {
		...
		if ( $type != '' ){
			...
			wp_enqueue_script( 'wp_review-js', trailingslashit( WP_REVIEW_ASSETS ) . 'js/main.js', array(), WP_REVIEW_PLUGIN_VERSION, true );
			...
			if ( $type == 'circle' ) {
				wp_enqueue_script( 'jquery-knob', trailingslashit( WP_REVIEW_ASSETS ) . 'js/jquery.knob.min.js', array(), '1.1', true );
			}
		}
	}
}
